export function formatAvailableTagsFromInitialService(data) {
    ...
    for (let i in data) {
        ...
        let item =data[i];
        let  obj = { isActive:Number(item.attributes.active), id: Number(item.id), title: item.attributes.name, iconClass: item.attributes.icon, description:item.attributes.description, color:item.attributes.icon_colour };
        arr.push(obj);
    }
    return arr;
}
