import configureMockStore from 'redux-mock-store';
import thunk from 'redux-thunk';
import * as actions from './staff';
import {
    RECEIVE_STAFF,
    TOGGLE_MANAGE_SECTION_MODAL_LOADING,
    ADDED_STAFF_RESPONSE,
    TOGGLE_STAFF_LOADING,
    CHANGE_STAFF_RESPONSE,
    DELETED_STAFF_RESPONSE
} from '../constants';
import { expect } from 'chai' ;// You can use any testing library
import axios from 'axios';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

axios.defaults.baseURL = 'https://httpbin.org/anything';

describe('Staff actions', function() {
    const sectionId = 1;
    const staffId = 5;
    const state = {
        response: {
            allIds: [1, 2]
        }
    };
    const responseId = state.response.allIds.length + 1;

    it('should call TOGGLE_MANAGE_SECTION_MODAL_LOADING -> ADDED_STAFF_RESPONSE -> TOGGLE_MANAGE_SECTION_MODAL_LOADING', function() {
        const store = mockStore(state);

        return store.dispatch(actions.addStaffResponse(sectionId, staffId))
            .then(() => {
                let actions = store.getActions();
                expect(actions[0]).to.eql({
                    type: TOGGLE_MANAGE_SECTION_MODAL_LOADING
                });

                expect(actions[1]).to.eql({
                    type: ADDED_STAFF_RESPONSE,
                    sectionId,
                    responseId,
                    staffId
                });

                expect(actions[2]).to.eql({
                    type: TOGGLE_MANAGE_SECTION_MODAL_LOADING
                });
            });
    });

    it('should call TOGGLE_STAFF_LOADING -> DELETED_STAFF_RESPONSE -> TOGGLE_STAFF_LOADING', function() {
        const store = mockStore();

        return store.dispatch(actions.deleteStaffResponse(sectionId, staffId, responseId))
            .then(() => {
                let actions = store.getActions();

                expect(actions[0]).to.eql({
                    type: TOGGLE_STAFF_LOADING,
                    staffId
                });

                expect(actions[1]).to.eql({
                    type: DELETED_STAFF_RESPONSE,
                    sectionId,
                    responseId
                });

                expect(actions[2]).to.eql({
                    type: TOGGLE_STAFF_LOADING,
                    staffId
                });
            });
    });

    it('should call TOGGLE_STAFF_LOADING -> CHANGE_STAFF_RESPONSE -> TOGGLE_STAFF_LOADING', function() {
        const store = mockStore();
        const responseId = 100;
        const status = 'Approved';

        return store.dispatch(actions.changeStaffResponse(staffId, responseId, status))
            .then(() => {
                let actions = store.getActions();

                expect(actions[0]).to.eql({
                    type: TOGGLE_STAFF_LOADING,
                    staffId
                });

                expect(actions[1]).to.eql({
                    type: CHANGE_STAFF_RESPONSE,
                    responseId,
                    status
                });

                expect(actions[2]).to.eql({
                    type: TOGGLE_STAFF_LOADING,
                    staffId
                });
            });
    });

    it('should call TOGGLE_MANAGE_SECTION_MODAL_LOADING -> RECEIVE_STAFF -> TOGGLE_MANAGE_SECTION_MODAL_LOADING', function() {
        const expectedActions = [
            TOGGLE_MANAGE_SECTION_MODAL_LOADING, 
            RECEIVE_STAFF,
            TOGGLE_MANAGE_SECTION_MODAL_LOADING
        ];

        const store = mockStore({ 
            staff: {
                needsFetching: true
            }
        });

        return store.dispatch(actions.fetchStaff()).then(() => { // return of async actions
            expect(store.getActions().map(action => action.type)).to.eql(expectedActions);
        });
    });
    
    
});